...
#=============================================
# error in truth-matching
#=============================================

parameters = ['R', 'z', 'm']
parameters_label = ['r [mm]', 'z [mm]', '#Deltam / m']
for p in range(len(parameters)):

    # define legends
    legends = [
                'MC Z\'#rightarrow%s, m_{Z\'} = 250 GeV, c#tau = 100 mm (tight)' % parameters[p],
                'MC Z\'#rightarrow%s, m_{Z\'} = 250 GeV, c#tau = 100 mm (loose)' % parameters[p],
                ]

    # initialize legend
    #legend = TLegend(0.27,0.73,0.88,0.93)
    legend = TLegend(0.30,0.83,0.70,0.93)
 
    # initialize canvas
    c= TCanvas("canvas_dv_err_%s" %parameters[p], "", 600, 600)
    ...
    histograms.append(f_m250t100.Get('truth/err/dv/tight/dv_%s_err_tight' % parameters[p]))
    histograms.append(f_m250t100.Get('truth/err/dv/loose/dv_%s_err_loose' % parameters[p]))

    # set histogram color 
    histograms[0].SetLineColor(color_tight)
    histograms[1].SetLineColor(color_loose)

    # set fill color
    #histograms[0].SetFillColor(color_tight)
    #histograms[1].SetFillColor(color_loose)
    ...
    for i in range(len(histograms)):
        ...
        histograms[i].GetXaxis().SetTitle("%s" % parameters_label[p])
    ...
    c.Print("output/dv_truth_matching_error_%s.pdf" % parameters[p])


#=============================================
# error in truth-matching (difference)
#=============================================

parameters = ['R', 'z', 'm']
parameters_label = ['#Deltar [mm]', '#Deltaz [mm]', '#Delta(#Deltam / m)']
for p in range(len(parameters)):
    ...
    m_hist_diff = histograms[1] - histograms[0]

    # set histogram color 
    m_hist_diff.SetLineColor(kBlack)

    histograms = []
    histograms.append(m_hist_diff)
    ...
    #c.SetLeftMargin(200)
    c.SetRightMargin(100)
    c.Print("output/dv_truth_matching_error_diff_%s.pdf" % parameters[p])
